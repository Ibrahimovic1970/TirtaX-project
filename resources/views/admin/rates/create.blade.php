...
@section('title', 'Tambah Tarif - TirtaX')
...
<div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <div class="bg-gradient-to-r from-navy-900 to-navy-800 rounded-2xl shadow-xl p-8 mb-8 text-white">
        <h1 class="text-3xl font-bold mb-2">➕ Tambah Tarif Baru</h1>
        <p class="text-sky-200">Tambahkan tarif pengiriman untuk rute baru.</p>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-2xl shadow-lg border border-sky-100 p-8">
        <form action="{{ route('admin.rates.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="service_type" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-navy-700 focus:border-navy-700 p-3 border text-gray-900">
                    ...
                    <option value="REGULER" {{ old('service_type') == 'REGULER' ? 'selected' : '' }}>Reguler (3-5 hari)</option>
                    <option value="EKONOMI" {{ old('service_type') == 'EKONOMI' ? 'selected' : '' }}>Ekonomi (5-7 hari)</option>
                    <option value="EXPRESS" {{ old('service_type') == 'EXPRESS' ? 'selected' : '' }}>Express (1-2 hari)</option>
                    <option value="SAMEDAY" {{ old('service_type') == 'SAMEDAY' ? 'selected' : '' }}>Same Day (Hari ini)</option>
                </select>
                @error('service_type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="price_per_kg" value="{{ old('price_per_kg') }}" required min="0" step="1000"
            ...
            <div>
                ...
                <input type="number" name="estimation_days" value="{{ old('estimation_days', 3) }}" required min="1" max="30"
